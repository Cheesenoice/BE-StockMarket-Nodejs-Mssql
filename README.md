...
## 13. Diagram (Mermaid + UML)

### 13.1 Mermaid kiến trúc tổng thể

```mermaid
flowchart LR
    C1[Nha dau tu Client]
    C2[Nhan vien Client]
    FE[Web UI or API Client]
    API[Node.js Backend\nExpress + Socket.IO]
    AUTH[Auth Middleware\nJWT + Role Guard]
    SVC[Service Layer\nNode mssql]
    SP[Stored Procedures\nsp_DangNhap sp_DatLenh sp_ProcessMatch ...]
    TRG[Triggers\ntrg_LenhDat_Validate\ntrg_LenhDat_UpdateBangGia]
    DB[(SQL Server\nQL_GiaoDichCoPhieu)]
...
### 13.2 UML Class Diagram (PlantUML)
...
package "Node Backend" {
  ...
  class AccountController {
    ...
    +getAccountDetailById(req, res)
    +saoKeGiaoDichLenh(req, res)
    +napTien(req, res)
    +rutTien(req, res)
  }

  class OrderControllerNDT {
    +placeOrder(req, res)
    +cancelOrder(req, res)
    +getPendingOrders(req, res)
  }

  class UserControllerNV {
  ...
  class StockControllerNV {
    +addStock(req, res)
    +updateStock(req, res)
    +deleteStock(req, res)
    +niemYetCoPhieu(req, res)
  }

  class AccountService {
    +getAccountsByNDT(user)
    +getAccountDetail(user, maTK)
    +napTien(user, data)
    +rutTien(user, data)
  }

  class OrderServiceNDT {
    +placeOrder(payload, user)
    +cancelOrder(payload, user)
  }

  class UserServiceNV {
    +createAccount(payload, user)
    +deleteAccount(payload, user)
  }

  class StockServiceNV {
    +addStock(user, payload)
    +deleteStock(user, maCP)
  }
...
database "SQL Server" as SQL {
  class StoredProcedures {
    ...
    +sp_DatLenh
    +sp_ProcessMatch
    +sp_KhopLenhDinhKyFull
    +sp_LayBangGiaTrucTuyen
  }
...
App --> OrderControllerNDT
App --> UserControllerNV
App --> StockControllerNV

AuthController --> DBConfig
AuthController --> AuthMiddleware

AccountController --> AccountService
OrderControllerNDT --> OrderServiceNDT
UserControllerNV --> UserServiceNV
StockControllerNV --> StockServiceNV

AccountService --> DBConfig
OrderServiceNDT --> DBConfig
UserServiceNV --> DBConfig
StockServiceNV --> DBConfig

DBConfig --> StoredProcedures
StoredProcedures --> SQL
Triggers --> SQL

PriceBoardSocket --> DBConfig
PriceBoardSocket --> StoredProcedures
@enduml
```
